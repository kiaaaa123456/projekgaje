<?php 
   ...
   for ($i=0; $i < $count; $i++) { 
      ...
      $company_id = 1;
      $nik = $hasil[$i]['nik'];
      $name = $hasil[$i]['fullname'];


      $email = $hasil[$i]['email'];
      $phone = $hasil[$i]['telpno'];
      $image = $hasil[$i]['photo'];
      if ($hasil[$i]['roleid'] == 1 || $hasil[$i]['roleid'] == 2) {
         $is_admin = 1;   
      } else {
         $is_admin = 0;
      }
      ...
      $sql_sync= "INSERT INTO users (id_user_monev, kdprov, kdkab, kdkec, kddesa, company_id, nik, name, email, phone, image, is_admin, role_id, department_id, shift_id, is_email_verified ,password, status_id, address, gender, birth_date, branch_id) VALUES ('$id_monev', '$kdprov', '$kdkab', '$kdkec', '$kddesa', '$company_id', '$nik', '$name', '$email', '$phone', '$image', '$is_admin', '$role_id', '$department_id', '$shift_id', '$is_email_verified', '$password', '$status_id', '$address', '$gender', '$birth_date', '$branch_id')";
      $statment_sync = $conn_->prepare($sql_sync);
      $statment_sync->execute();

   }

   echo "Proses Singkronisasi User Selesai";
