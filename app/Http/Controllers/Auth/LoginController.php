    ...
    */
    ...
    public function authenticate(Request $request)
    {
        ...
        try {
            ...
            if ($credentials['is_email']) {
                $type = 'email';
                $input = ['email' => $credentials['email'], 'password' => $credentials['password'], 'aktif' => 1];
            } else {
                $type = 'phone';
                $input = ['phone' => $credentials['email'], 'password' => $credentials['password'], 'aktif' => 1];
            }

            $user = User::where($type, $request->email)->first();

            if ($user->aktif==0) {
                return response()->json(['error' => ['email' => ["User tidak aktif"]]], 422);
            }
            ...
            if ($user) {
                ...
                return response()->json(['error' => ['email' => ["User tidak ditemukan"]]], 422);
            }
        } catch (\Throwable $th) {
            Log::error($th->getMessage());
            return response()->json(['error' => ['email' => ["Ada kesalahan sepertinya"]]], 422);
        }
    }
    public function userLogin($input, $user, $uuid)
    {
        ...
        if (Auth::attempt($input)) {
            ...
            return response()->json(['error' => ['email' => ["Kombinasi email dan password tidak sesuai"]]], 422);
        }
    }
