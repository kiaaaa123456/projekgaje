        ...
        try {
            ...
            if (in_array('appointments', $screen_data)) {
                $data['today'][] = [
                    ...
                    //     })
                    //     ->count(),
                    'number' => Appoinment::whereMonth('date',$month)
                    ->where(function ($query) {
                        $query->where('created_by', auth()->user()->id)
                            ->orWhere('appoinment_with', auth()->user()->id);
                    })
            ...
            if (in_array('visit', $screen_data)) {
                $data['today'][] = [
                    ...
                    'title' => 'Visit',
                    'slug' => 'visit',
                    'number' => Visit::where('year', $year)->where('user_id', auth()->user()->id)->count(),
                ];
            }
            ...
            $data['current_month'][] = [
                ...
                'title' => 'Izin',
                'slug' => 'izin',
                'number' => str_replace(' days', '', $attendance_data['izin']),
            ];
            $data['current_month'][] = [
                'image' => asset($this->getStatisticsImage('absent')),
                'title' => 'Sakit',
                'slug' => 'sakit',
                'number' => str_replace(' days', '', $attendance_data['sakit']),
            ];
    ...
    public function getNewDashboardStatistics($request)
    {
        try {
            ...
            $data['today'][] = [
                ...
                'number' => number_format_short(DB::table('monthly_report')->where('company_id', auth()->user()->company_id)->where('user_id', auth()->id())->count()),
            ];
            $data['today'][] = [
                'image' => $this->getNewStatisticsImage('appointment'),
                'title' => _trans('dashboard.Total Appointments'),
                'color_class' => 'circle-danger',
                'number' => number_format_short(DB::table('daily_activity')->where('company_id', auth()->user()->company_id)->where('created_by', auth()->id())->count()),
            ];
            return $this->responseWithSuccess("Dashboard Statistics Data", $data, 200);
        } catch (\Throwable $exception) {
            return $this->responseWithError($exception->getMessage(), [], 500);
