...
class UserRepository
{
    ...
    {{
        ...
        if ($request->kodeprov) {
            $where[] = ['kdprov', '=', $request->kodeprov];
        }
        if ($request->kodekab) {
            $where[] = ['kdkab', '=', $request->kodekab];
        }
        if ($request->kodekec) {
            $where[] = ['kdkec', '=', $request->kodekec];
        }
        if ($request->kodedesa) {
            $where[] = ['kddesa', '=', $request->kodedesa];
        }
        if (@$request->designation) {
            $where[] = ['designation_id', $request->designation];
        }
        ...
        return [
            'data' => $data->map(function ($data) {
                ...
                if (@$data->role_id == 4) {
                    $status_user = '<small class="badge badge-success">Aktif</small>';
                } else if (@$data->role_id == 10) {
                    $status_user = '<small class="badge badge-danger">Ditolak</small>';
                } else {
                    $status_user = '<small class="badge badge-warning">Menunggu</small>';
                }
                $user_image = '';
                $user_image .= '<img data-toggle="tooltip" data-placement="top" title="' . $data->name . '" src="' . uploaded_asset($data->avatar_id) . '" class="staff-profile-image-small" >';
                ...
                return [
                    ...
                    'status' => $status_user,
                    // 'action' => $button,
                    'action' => '<i class="fa fa-edit edit" style="cursor:pointer" title="Edit Data" id="'.$data->id.'"></i>&nbsp;&nbsp;<i class="fa fa-trash delete" style="cursor:pointer" title="Hapus Data" id="'.$data->id.'"></i>',
                ];
            }),
            'pagination' => [
                'total' => $data->total(),
