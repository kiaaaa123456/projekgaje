...
class locationRepository
{
        ...
        return datatables()->of($content->latest()->get())
            ->addColumn('action', function ($data) {
                ...
                if (hasPermission('location_delete')) {
                    $action_button .= actionButton('Delete', '__globalDelete(' . $data->id . ',`hrm/location/delete/`)', 'delete');
                }
    ...
    function create($request)
    {
        try {
            ...
            if ($location_bind) {
                return $this->responseWithError(_trans('message.Location already exists'), 'name', 422);
            }
