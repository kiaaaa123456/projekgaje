...
class AttendanceReportRepository
{
    ...
    public function singleAttendanceSummary($user, $request)
    {
        ...
        if ($request->month) {
            ...
            $monthArray = $this->getCurrentMonthDays();
                ...
                $totalAbsent += Attendance::where('user_id',$user->id)->whereNotNull('check_in')->whereNotNull('check_out')->whereMonth('date',$currentMonth)->whereYear('date',$currentYear)->count();
        foreach ($monthArray as $day) {
            ...
            $leaveIzin = LeaveRequest::where(['leave_requests.company_id' => $user->company->id, 'leave_requests.user_id' => $user->id, 'leave_requests.status_id' => 1, 'assign_leaves.type_id' => 4])
                ->leftJoin('assign_leaves','leave_requests.assign_leave_id','assign_leaves.id')
                ->where('leave_from', '<=', $todayDateInSqlFormat)
                ->where('leave_to', '>=', $todayDateInSqlFormat)
                ->first();

            $leaveCountedAlreadyIzin = 0;
            if ($leaveIzin) {
                $leaveCountedAlreadyIzin = 1;
                $totIzin += 1;
            }
            
             $leaveSakit = LeaveRequest::where(['leave_requests.company_id' => $user->company->id, 'leave_requests.user_id' => $user->id, 'leave_requests.status_id' => 1, 'assign_leaves.type_id' => 5])
                ->join('assign_leaves','leave_requests.assign_leave_id','assign_leaves.id')
                ->where('leave_from', '<=', $todayDateInSqlFormat)
                ->where('leave_to', '>=', $todayDateInSqlFormat)
                ->first();

            $leaveCountedAlreadySakit = 0;
            if ($leaveSakit) {
                $leaveCountedAlreadySakit = 1;
                $totSakit += 1;
            }
        ...
        $data['izin'] = "{$totIzin} days";
        $data['sakit'] = "{$totSakit} days";
            ...
            switch ($request->type) {
                case 'present':
                    $data['attendance'] = $this->attendance->query()
                        ...
                        ->select('users.id', 'users.name', 'users.avatar_id', DB::raw("DATE_FORMAT(check_in, '%h:%i %p') as check_in"), DB::raw("DATE_FORMAT(check_out, '%h:%i %p') as check_out"))
                        ->get();
                    break;
                case 'absent':
                ...
                case 'leave':
                    ...
                    $data['attendance'] = User::query()
                        ...
                        ->select('leave_requests.*', 'users.id', 'users.name', 'users.avatar_id')
                        ->get();
                    break;
    ...
    function CheckInTable($data)
    {
        ...
        if (is_array($checkInStatus) && !empty($checkInStatus)) {
            ...
            $status .= '<div class="d-flex badge-responsive">';
            $status .= '<span class="badge badge-success">' . $this->dateTimeInAmPm($data->check_in) . '</span>';
            $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $data->check_in_location . '"> <i class="fa fa-map"></i> </span>';
            $status .= '</div>';
    ...
    function checkOutTable($data)
    {
        if ($data->check_out) {
            ...
            if (is_array($checkOutStatus) && !empty($checkOutStatus)) {
                ...
                $status .= '<div class="d-flex badge-responsive">';
                $status .= $data->check_out ? '<span class="badge badge-danger">' . $this->dateTimeInAmPm($data->check_out) . '</span>' : '';
                $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $data->check_out_location . '"> <i class="fa fa-map"></i> </span>';
                $status .= '</div>';
