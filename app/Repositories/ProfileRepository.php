...
class ProfileRepository
{
    ...
    public function getProfileDetails($request, $id): \Illuminate\Http\JsonResponse
    {
        ...
        if ($user) {
            $province = DB::table('wilayah_provinsi')->where('kdprov',$user->kdprov)->first();
            $regency = DB::table('wilayah_kabupaten')->where('kdkab',$user->kdkab)->first();
            $subdistrict = DB::table('wilayah_kecamatan')->where('kdkec',$user->kdkec)->first();
            $village = DB::table('wilayah_desa')->where('kddesa',$user->kddesa)->first();
            ...
            $data['province'] = $province->nmprov ?? null;
            $data['regency'] = $regency->nmkab ?? null;
            $data['subdistrict'] = $subdistrict->nmkec ?? null;
            $data['village'] = $village->nmdesa ?? null;
