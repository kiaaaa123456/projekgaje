...
class SupportTicketRepository
{
    ...
    public function store($request): \Illuminate\Http\JsonResponse
    {
        ...
        // }
        $support->user_id = auth()->id();
        $support->company_id = 1;
        $support->date = date('Y-m-d');
        $support->year = $request->year;
        $support->month = $request->month;
        $support->status = 0;
        ...
        $support->file = $upload_path . $profileImageSaveAsName;
    ...
    public function suportDatatable($tickets)
    {
        return datatables()->of($tickets->latest()->get())
            ...
            ->rawColumns(array('date', 'year', 'month', 'subject', 'employee_name', 'type', 'priority', 'action'))
            ->make(true);
    }
