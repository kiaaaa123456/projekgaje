...
class ExpenseRepository
{
    ...
    {
        $params = [];
        $content = $this->model->query()->with('user:id,name', 'category:id,name', 'payment:id,name,class,color_code', 'status:id,name,class,color_code')
        ...
        return datatables()->of($content->latest()->get())
            ->addColumn('action', function ($data) {
                ...
                if (hasPermission('expense_delete') && $data->status_id != 5 && @$data->pay == 9) {
                    $action_button .= actionButton('Delete', '__globalDelete(' . $data->id . ',`hrm/expenses/delete/`)', 'delete');
                }
        ...
        try {
            ...
            if ($request->hasFile('attachment')) {
                ...
                if ($expense->attachment) {
                    // If there was an existing attachment, delete it first
                    $this->deleteImage(asset_path($expense->attachment));
                    $expense->attachment = null; // Remove the attachment reference from the expense
                }
    
                // Upload the new attachment and store its reference in the database
                $attachment = $this->uploadImage($request->attachment, 'expense')->id;
