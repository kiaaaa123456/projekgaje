...
class NoticeRepository
{
    ...
    public function __construct(Notice $notice, NoticeViewLog $noticeViewLog, DepartmentRepository $departmentRepo, ExpenseService $service)
    {
        $this->notice = $notice;
    ...
    public function storeNotice($request)
    {
        ...
        try {
            //string to array
            if (is_string($request->department_id)) {
                $request['department_id'] = explode(',', $request->department_id);
            }
            if (in_array(0, $request->department_id)) {
                $request['department_id'] = Department::where('company_id', $this->companyInformation()->id)->pluck('id')->toArray();
            }
            ...
            foreach ($request->department_id as $item) {
                ...
                if($department && $department->users->count()>0){
                    $this->sendChannelFirebaseNotification('department'.$department->id, 'notice', '', 'notice', $request->subject, $request->description, $notice->attachment_file_id?uploaded_asset($notice->attachment_file_id):null);
                    ...
                    foreach ($department->users as $user) {
                        $details = [
                            ...
                            'body' => $notice->description,
                            'actionText' => 'View',
                            'image' => $notice->attachment_file_id?uploaded_asset($notice->attachment_file_id):null,
                            'actionURL' => [
                                'app' => 'notice',
                                'web' => '',
                                'target' => '_blank',
            ...
            if($request->is('api/*')){
                ...
                Toastr::success(_trans('Notice created successfully'), 'Success');
            ...
            if($request->is('api/*')){
                ...
                Toastr::error(_trans('Something went wrong'), 'Error');
                return redirect()->route('notice.index');
            }
        }
    }
