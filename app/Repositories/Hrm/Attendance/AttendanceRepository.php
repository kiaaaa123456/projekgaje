...
class AttendanceRepository
{
    ...
    public function getCheckInCheckoutStatus($userId)
    {
        ...
        if ($user) {
            $default_timezone = date_default_timezone_set("".$user->time_zone."");
            // Log::info('Default Timezone: ' . $default_timezone);
            // Log::info('Default Timezone checkin check: ' .$user->time_zone);
            ...
            if ($attendance) {
                ...
                if ($attendance->check_out && $attendance->date == $current_date) {
                     $carbonInstance = Carbon::parse($in_time);
                     $timeOnlyin_time = $carbonInstance->format('H:i:s');

                     $carbonInstanceout = Carbon::parse($out_time);
                     $timeOnlyout_time = $carbonInstanceout->format('H:i:s');
                    // original format 00 Hr's:11 min
                    preg_match('/(\d+) Hr\'s:(\d+) min/', $stay_time, $matches);

                    if (count($matches) === 3) {
                        // Convert to 24-hour format using Carbon
                        $time = Carbon::createFromTime($matches[1], $matches[2])->format('H:i');
                        if (Carbon::parse($time)->diffInHours(Carbon::today()) > 8) {
                            // If it exceeds, set the time to 8 hours
                            $time = '08:00';
                        }
                    ...
                    return $this->responseWithSuccess('Already checked out', [
                        ...
                        'checkin' => true,
                        'checkout' => true,
                        'in_time' => @$timeOnlyin_time ?? '',
                        'out_time' =>  @$timeOnlyout_time ?? '',
                        'stay_time' => $time
                    ], 200);
                } elseif(!$attendance->check_out && !$checkout_status){
                    $carbonInstance = Carbon::parse(@$attendance->check_in);
                    $timeOnly = $carbonInstance->format('H:i:s');
    ...
    public function userAttendance($user, $request)
    {
        if ($user) {
            ...
            $attendanceCount = $this->attendance->where(['user_id' => $user->id, 'date' => $request->date])->count();
            ...
            // Log::info('Default Timezone checkin masuk: ' .$user->time_zone);
            $isIpRestricted = $this->isIpRestricted();
            if ($isIpRestricted && $attendanceCount == 0) {
                ...
                if (count($attendance_status) > 0) {
                    ...
                    return $this->responseWithSuccess('Absen masuk berhasil', $check_in, 200);
                } else {
    ...
    public function storeInout($request)
    {
        ...
        try {
            ...
            return $this->userAttendanceInout($user, $request);
        } catch (\Throwable $th) {
            Log::error($th);
            return $this->responseWithError(_trans('response.Something went wrong.'), [$th->getMessage()], 400);
        }
    }
    public function userAttendanceInout($user, $request)
    {
        if ($user) {
            $attendance = $this->attendance->where(['user_id' => $user->id, 'date' => $request->date])->first();
            
            if(isset($attendance->check_in) != null){
                $check_out = $this->attendance->query()->find($attendance->id);
                    if ($check_out && $request->checkout != '0000-00-00 00:00:00' && $request->checkout != '00:00' && $request->checkout != '0000-00-00' && $request->checkout != '00:00:00') {
                        
                        $request['checkin_ip'] = getUserIpAddr();
                        $check_out->check_out = $request->checkout;
                        $check_out->checkout_ip = $request->checkin_ip;
                        $check_out->check_out_location = getGeocodeData($request->check_out_latitude,$request->check_out_longitude);
                        $check_out->check_out_latitude = $request->check_out_latitude;
                        $check_out->check_out_longitude = $request->check_out_longitude;
                        $check_out->check_out_city = $request->city;
                        $check_out->check_out_country_code = $request->country_code;
                        $check_out->check_out_country = $request->country;
                        $check_out->save();
                    }
            }
            $isIpRestricted = $this->isIpRestricted();
            if ($isIpRestricted && !$attendance && $request->checkout != '0000-00-00 00:00:00' && $request->checkout != '00:00' && $request->checkout != '0000-00-00' && $request->checkout != '00:00:00') {
                ...
                $request['checkin_ip'] = getUserIpAddr();
                    ...
                    $check_in->remote_mode_in = $request->remote_mode_in;
                    $check_in->date = $request->date;
                    $check_in->check_in = $request->checkin;
                    $check_in->check_out = $request->checkout;
                    $check_in->in_status = '';
                    $check_in->checkin_ip = $request->checkin_ip;
                    $check_in->late_time = '';
                    // $check_in->check_in_location = $request->check_in_location;
            ...
            }else if ($isIpRestricted && !$attendance) {
            ...
            return $this->responseWithSuccess('Absen masuk berhasil', $request, 200);
        } else {
            return $this->responseWithError('No user found', [], 400);
        }
    }
        ...
        try {
            ...
            if ($user) {
                ...
                if ($isIpRestricted) {
                    ...
                    if (count($attendance_status) > 0 && $request->checkout != '0000-00-00 00:00:00' && $request->checkout != '00:00' && $request->checkout != '0000-00-00' && $request->checkout != '00:00:00') {
                        ...
                        date_default_timezone_set("".$user->time_zone."");
                        // Log::info('Default Timezone checkin keluar: ' .$user->time_zone);
                        $check_in = $this->attendance->query()->find($id);
                        if(!$check_in){
                            Log::info('checkin data: ' .$check_in);
                            Log::info('checkin id: ' .$id);
                        }    
                        $current_date_time = date('Y-m-d H:i:s');
                        $check_current_date_time = date('Y-m-d H:i');
                        $carbonDate = Carbon::createFromFormat('Y-m-d H:i:s', $check_in->check_in);
                        $formattedDate = $carbonDate->format('Y-m-d H:i');
                        // Log::info('checkin before: ' .$formattedDate);
                        if ($check_in && $current_date_time != '0000-00-00 00:00:00' && $formattedDate != $check_current_date_time ) {
                            ...
                            return $this->responseWithSuccess('Absen pulang berhasil', $check_in, 200);
                        } else {
        ...
        try {
            ...
            if ($user) {
                ...
                if (count($attendance_status) > 0) {
                    ...
                    return $this->responseWithSuccess('Absen pulang berhasil', $check_out, 200);
                } else {
                    return $this->responseWithError('No Schedule found', [], 400);
                }
            } else {
    ...
    function fields()
    {
        return [
            _trans('common.No'),
            _trans('common.Nama '),
            _trans('common.Tanggal'),
            _trans('common.Peran'),
            _trans('common.Masuk'),
            _trans('common.Pulang'),
            // _trans('common.Jam Kerja'),
