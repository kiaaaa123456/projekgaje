...
class LeaveRequestRepository
{
    ...
    public function leaveDetailsInformation($request, $id)
    {
        ...
        if ($user) {
            ...
            if ($leaveRequest) {
                ...
                $variable = @$leaveRequest->status->name;
                ...
                $getFile = Upload::where('id',$leaveRequest->attachment_file_id)->first();
                $data['attachment'] = 'http://tekad.kemendesa.go.id/e-lapkin/'.$getFile->img_path ?? ''; 
    ...
    public function store($request)
    {
        ...
        try {
            $user = $this->getUserById($request->user_id);  
            $check = LeaveRequest::where('user_id',$user->id)->where('leave_from',$request->leave_from)
            ->where('leave_to',$request->leave_to)->where('reason',$request->reason)->count();
            $checkTotal = AssignLeave::whereId($request->assign_leave_id)->first();
            $totalUseg = $checkTotal->days ?? 0;
            if($checkTotal->type_id == 3){
                $totalUseg = 1000;
            }
            $checkOld = LeaveRequest::where('user_id', $user->id)
            ->where('assign_leave_id', $request->assign_leave_id)
            ->whereIn('status_id', [1, 2])
            ->sum('days');
           
            if($check == 0 && $totalUseg >= $checkOld){
                        ...
                        if ($user_check || $department_check) {
                            ...
                            if ($user) {
                                ...
                                if ($request->hasFile('attachment_file')) {
                                    ...
                                    $profileImage = $request->file('attachment_file');
                                    ...
                                    $leaveRequest->attachment_file_id  = DB::select( DB::raw("SELECT max(id) AS id FROM uploads"))['0']->id;
                                ...
                                $leaveRequest->author_info_id = $author->id ?? Auth::user()->id;
                                $leaveRequest->save();
                                ...
                                return $this->responseWithSuccess('Izin tidak masuk kerja berhasil di buat', [], 200);
                            } else {
            ...
            }else if ($check == 0 && $checkTotal->days <= $checkOld){
                return $this->responseWithSuccess('Kouta tidak masuk kerja telah habis', [], 200);

            }else{
                return $this->responseWithSuccess('Izin tidak masuk kerja telah di buat', [], 200);
            }
    ...
    public function availableLeave($request)
    {
        ...
        if ($user) {
            ...
            foreach ($assignedLeaveList as $key => $item) {
                ...
                $data[$key]['leave_use'] = intval($approvedLeave);
            }
    ...
    {        
        return [
            ...
            _trans('common.Hari'),
            _trans('common.Tanggal Tersedia'),
            _trans('common.Pengganti'),
            // _trans('common.Approval'),
            // _trans('common.HR Approved'),
            _trans('common.File'),
            _trans('common.Status'),
            _trans('common.Aksi'),
        ...
        try {
            ...
            return [
                'data' => $data->map(function ($data) {
                    ...
                    if(@$data->substitute->name==null) {
                        $pengganti = '-';
                    } else {
                        $pengganti = @$data->substitute->name;
                    }
                    ...
                    return [
                        ...
                        'date' =>  date('d-m-Y', strtotime($data->leave_from)),
                        'days' => $data->days,
                        'available_days' => $available_leave,
