    ...
    public function leaveSummary($request=null)
    {
        ...
        if ($user) {
            ...
            if(settings('leave_assign')==1){
                $assignedLeaveList = AssignLeave::where(['company_id' => $this->companyInformation()->id, 'user_id' => $user->id])
                ->whereNot('type_id', 5)
                ->get();
            }else{
                $assignedLeaveList = AssignLeave::where(['company_id' => $this->companyInformation()->id, 'department_id' => $user->department_id])
                ->whereNot('type_id', 5)
                ->get();
            }
            $data = [];
