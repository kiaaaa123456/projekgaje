...
<!-- MODAL -->
    <div id="formConfirmPulang" class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myModalLabel">Hapus Absen Pulang</h5>
                    <button type="button" class="close close-modal" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-danger" role="alert">
                    Apakah Anda ingin menghapus absen pulang pada data ini ?
                    </div>
                    <form method="post" id="delete_form_pulang" class="form-horizontal" enctype="multipart/form-data">
                    @csrf
                        <input type="hidden" name="id_delete" id="id_delete_pulang" />
                        <center><span class="loader"><i class="fa fa-spinner fa-3x fa-spin"></i></span></center>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary waves-effect close-modal" data-dismiss="modal">Batal</button>
...
<script type="text/javascript">
    ...
    function loadData(page, limit, search, date, kodeprov, kodekab, kodekec, kodedesa){
        ...
        $.ajax({
        url: "absensi-admin/"+page+'/'+limit+'/'+search+'/'+date+'/'+kodeprov+'/'+kodekab+'/'+kodekec+'/'+kodedesa,
        type: 'get',
        dataType: 'json',
                ...
                if (response['status']!='success') {
                    ...
                    if(len > 0){
                        ...
                        for(var i=0; i<len; i++){
                            ...
                            "<td>" + nmkec + "</td>" +
                            "<td>" + nmdesa + "</td>" +
                            "<td><i class='fa fa-trash delete' style='cursor:pointer' title='Hapus Data' id='"+response['data'][i].id_main+"'></i>&nbsp;&nbsp;&nbsp;<i class='fa fa-repeat att-pulang' style='cursor:pointer' title='Hapus Absen pulang' id='"+response['data'][i].id_main+"'></i></td>" +
                            "</tr>";
    ...
    $(document).on('click', '.att-pulang', function(){
        var id = $(this).attr('id');
        $('#id_delete_pulang').val(id);
        $('#formConfirmPulang').modal('show');
        $('.loader').css("visibility", "hidden");
    });
    ...
    $('#delete_form_pulang').on('submit', function(event){
        event.preventDefault();
        $.ajax({
            url:"delete-absensi-pulang",
            method:"POST",
            data: new FormData(this),
            contentType: false,
            cache:false,
