...
@section('title', 'Laporan Absensi Bulan Ini')
@section('content')

	<div class="breadcrumb-warning d-flex justify-content-between ot-card">
    <div><h3>Laporan Absensi Bulan Ini</h3></div>
    <nav aria-label="breadcrumb ">
        <ol class="breadcrumb ot-breadcrumb ot-breadcrumb-basic">
            <li class="breadcrumb-item"><a href="{!!url('dashboard')!!}">Dashboard</a></li>
            <li class="breadcrumb-item active">Laporan Absensi Bulan Ini</li>
...
<script type="text/javascript">
                ...
                if (response['status']!='success') {
                    ...
                    if(len > 0){
                        ...
                        for(var i=0; i<len; i++){
                            ...
                            "<td><a class=''><i class='fa fa-eye showAbsensi' id='"+response['data'][i].id_main+"' style='cursor:pointer' title='Preview Absensi'></i></a>&nbsp;<a href='laporan-absensi/"+response['data'][i].id_main+"'><i class='fa fa-download' style='cursor:pointer' title='Download Absensi' id='"+response['data'][i].id_main+"'></i></a></td>" +
                            "</tr>";
    ...
    $(document).on('click', '.close-modal', function(){
     ...
     $(document).on('click', '.showAbsensi', function(){
        $('#content-absensi').empty();
        var id = $(this).attr('id');
        var content = '<iframe src="https://tekad.kemendesa.go.id/e-lapkin/get-preview-absensi/'+id+'" width="100%" height="450px"></iframe>'
        $('#content-absensi').append(content);
